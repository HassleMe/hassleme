...
my %CONFIG = Config::General->new( __DIR__ . '/conf/general' )->getall;
%Hassle::CONFIG = %CONFIG;
...
<html>
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="/hassleme.css">
    <link rel="stylesheet" type="text/css" href="//fonts.googleapis.com/css?family=Source+Sans+Pro:400,600">
    <link rel="stylesheet" type="text/css" href="/assets/css/banner.css">
  </head>
  <body>
EOSTART_HTML

    my $staging = $CONFIG{HM_STAGING} ? 1 : 0;
    if ($staging) {
...
sub hassle_form {
    ...
    print $q->div({id=>'createHassleBox'}, [
              $q->form({
                  name => 'createHassleForm',
                  action => '/',    # XXX
                  method => 'POST',
                  enctype => 'application/x-www-form-urlencoded',
                  accept_charset => 'UTF-8',
              }, [
              $q->input({ type => 'hidden', name => 'fn', value => 'home'}),
              $q->h2('Set up a hassle now!'),
              $q->p([
                    'Hassle me <b>roughly</b> every ',
                    $q->input({ type => 'text', name => 'freq', size => 3 }),
              ...
              $q->p([
                    'Send the emails to: ',
                    $q->input({ name => 'email', size => 25, type => 'text' }),
                    ]),
              $q->ul([
              ...
              $q->input({ type => 'submit', value => 'Set up this hassle now >>' }),
              ])],
          );
...
<a href="http://www.mysociety.org/">mySociety</a> tool.
<br>
</p>
...
sub fn_home {
    ...
    my $emails = $req->parameters->{email};
    my $freq   = $req->parameters->{freq};
    my $units  = $req->parameters->{units};
    my $what   = $req->parameters->{what};
    my $public = lc $req->parameters->{public};
    ...
    if ($req->method eq 'POST') {
        ...
        if (!@errors) {
            ...
            dbh()->do('
                    ...
                    values (?, ?, ?, ?, ?)', {},
                    $hassle_id, $freq, $what, $public, $req->remote_host());
            ...
            foreach my $email (@emails) {
                ...
                my $confirmurl = $CONFIG{WEBURL} . "/C/" . token($recipient_id);
                warn "RARR!!  $confirmurl";
...
EOF
            ...
            hassle_header($fn, 'Now check your email!');
...
sub make_psgi {
    ...
    return sub {
        ...
        my $res = $req->new_response(200, {}, $output);
        return $res->finalize;
    }
}

# http://hassleme.co.uk/C/6X5861Xd980b2862b4a
sub fn_confirm {
    my ($req, $q, $fn) = @_;
    my $token = $req->path_info;
    $token =~ s{^/}{};
...
<p>
Great!  We'll now hassle you as per your request.
</p>
</div>
EOF
    } else {
...
sub fn_hassles {
    my ($req, $q, $fn) = @_;
    my $parameter = $req->path_info;
    my $longest = $parameter eq '/longest';
...
EOF
    ...
    while (my @row = $sth->fetchrow_array) {
        ...
        print "<b>" . $q->entity_encode($what) . "</b> roughly every " . $frequency . " " . ($frequency > 1 ? "days" : "day");
...
<dd>Please email <a
href="mailto:team\@hassleme.co.uk">team\@hassleme.co.uk</a> with a
...
developers and volunteers at mySociety to post frequently to their blog, letting
...
href="http://www.writetothem.com/">WriteToThem.com</a>.
...
sub fn_terms {
    my ($req, $q, $fn) = @_;
    hassle_header($q, 'Terms and Conditions');
    my $vars = {};
    my $params = $req->path_info;
    $params =~ s{^/}{};
    my ($token, $action) = split '/', $params;
...
builder {
    enable "Plack::Middleware::Static",
        path => qr{\.(png|jpg|css|html)$}, root => 'web/';
    enable "Plack::Middleware::Static",
        path => qr{/assets}, root => '.';
    mount '/'        => make_psgi('home', \&fn_home);
    mount '/create'  => make_psgi('home', \&fn_home);

    # /hassles  /hassles/{longest}
    mount '/hassles' => make_psgi('hassles', \&fn_hassles);

    mount '/faq'     => make_psgi('faq', \&fn_faq);
    mount '/privacy' => make_psgi('privacy', \&fn_privacy);

    # /C/{token}  /U/{token}
    mount '/C',      => make_psgi('confirm', \&fn_confirm);
    mount '/U',      => make_psgi('confirm', \&fn_unsubscribe);

    # /T/{token}  /TT/{token}/{action}
    mount '/T'       => make_psgi('terms', \&fn_terms);
    mount '/TT'      => make_psgi('terms', \&fn_terms);
};
